...
namespace ValveSDK
{
	...
	public:
		...
		int GetWeaponID()
		{
			return *(int*)((DWORD)this + /*0x2D70 + 0x2DB0 + 0x2F88*/0x2F88); //m_AttributeManager + m_Item + m_iItemDefinitionIndex
		//^^ just use the fucking m_iItemDefinitionIndex by itself and there are no issues ~ wippydog
		}
		...
		bool IsPistol()
		{
			...
			return (iWeaponID == glock || iWeaponID == p2000
				...
				|| iWeaponID == fiveseven || iWeaponID == usp_s);
		}
		...
		{
			int iWeaponID = GetWeaponID();
			return (iWeaponID == knife || iWeaponID == 59 || iWeaponID == 507
				|| iWeaponID == 500 || iWeaponID == 509 || iWeaponID == 506
				|| iWeaponID == 516 || iWeaponID == 515 || iWeaponID == 505
				|| iWeaponID == 508 || iWeaponID == 512 || iWeaponID == 514);
		}

		bool IgnoreRCS()
		{
			int iWeaponID = GetWeaponID();
			return (iWeaponID == awp || iWeaponID == ssg08);
		}
	};
}