...
public:
	...
	void AddElement(int iType, int add_to_x, const char *pszElementName, const char *pszComment, float *cvar, float min, float max, float step, float fToValue, int increment, static bool &keybind);
	...
	void SetColorDummy(int r, int g, int b, int a)
	{
		fColDummy[0] = r;
		fColDummy[1] = g;
		fColDummy[2] = b;
